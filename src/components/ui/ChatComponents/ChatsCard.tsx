  ...
  return (
    ...
    >
      <div className="relative h-9 w-9 rounded-[50%]">
        ...
        <div
          className={`absolute -top-1 right-0 size-3 rounded-full  ${item.status === "request" ? "bg-green-500" : item.status === "cancel" ? "bg-rose-600" : item.status === "block" ? "bg-yellow-500" : "bg-purple-500"}`}
        ></div>
      </div>
      ...
      <div className="relative col-span-1 flex justify-end">
        ...
        {showSetting && (
          <div
            ref={boxRef}
            className={`border-gray-300 absolute -top-4 right-6 flex w-40 flex-col items-start justify-center rounded-md rounded-ee-none rounded-es-2xl rounded-se-2xl border border-solid bg-white  p-2 text-left  shadow `}
          >
            <div key="text">
